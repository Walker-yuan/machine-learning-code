...
train = open(sys.argv[1], encoding='utf-8')
# test = open(sys.argv[2])
test = open(sys.argv[2], encoding='utf-8')
mode = sys.argv[3]
...
# calculate CMI(conditional mutual information) between i feature and j feature, traverse all possible values and classes
...
def laplace_estimate_possibility(feature_column_with_same_class, feature_value, amount_of_values_combination):
    ...
    return float(num + 1) / (len(feature_column_with_same_class) + amount_of_values_combination) # 即书上的(7.20)式
    ...
    for i in range(len(parent_feature)):
            for j in range(len(child_feature)):
                for k in range(len(class_list)):
                    ...
                    key = str(j) + str(k) + str(i) # 先child，之后class，最后parent
